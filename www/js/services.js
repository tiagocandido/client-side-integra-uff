...
.factory('Accounts', function(DB) {
    ...
    self.create = function(system, login, password){
      return DB.query('INSERT INTO accounts(system, login, password) VALUES(?, ?, ?);',[system, login, password])
    };
...
//  const autenticationPath = 'http://localhost:3000/conexao_uff/authentication/login';
  const autenticationPath = 'http://localhost:8100/api/login';
  self = {
    login: function(credentials){
      return $http
          .post(autenticationPath, credentials)
          .then(function(response){
            window.localStorage['token_iduff'] = response.data.token;
